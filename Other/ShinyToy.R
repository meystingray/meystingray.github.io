...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            fluidRow(column(12, selectInput("COLUMN", "Filter By:", choices = colnames(iris))),
                     column(12, selectInput("CONDITION", "Boolean", choices = c("==", "!=", ">", "<"))),
                     column(12, uiOutput("COL_VALUE")))
        ),
...
server <- function(input, output) {
    ...
    #ifelse(class(iris[,input$column]) %in% c("factor","character"),"'","")
    
    filtering_string <- reactive ({
        paste0("filter(iris, ", input$COLUMN, " ", input$CONDITION, " ", 
               "'", input$VALUE, "'", ")")
    })
